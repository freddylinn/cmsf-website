      ...
      {isFirstInVisibleGroup && (
        <th rowSpan={visibleGroupSpan} className="w-32 border border-slate-700 bg-slate-50 p-4 align-middle">
          <div className="has-tooltip relative flex flex-col items-center">
            ...
            <button className="-rotate-90 print:hidden px-2 py-0.5 mb-2 rounded bg-sky-100 text-[11px] text-sky-700 font-bold border border-sky-200">i</button>
          </div>
          <p className="rotate-180 mx-auto text-xs font-black text-slate-900 uppercase" style={{ writingMode: "vertical-rl" }}>{group}</p>
        </th>
      )}

      <td className={`p-4 border border-slate-700 text-sm text-left ${isChecked ? 'bg-white font-black text-slate-900 underline decoration-sky-100 decoration-4' : 'bg-slate-50 text-slate-900 font-bold'}`}>
      ...
      <td className={`p-4 border border-slate-700 ${isChecked ? 'bg-white' : 'bg-slate-50'}`}>
        <div className="flex flex-col items-center">
          <input
            type="checkbox"
            checked={isChecked}
            className="h-6 w-6 text-sky-500 rounded border-slate-400 cursor-pointer"
            onChange={() => onToggle(!isChecked)}
          />
          {/* USE DIRECTIONAL: Fixes ESLint 'unused-vars' error */}
          {isChecked && directional && (
            <div className="flex justify-center mt-3 gap-4 border-t-2 border-slate-100 pt-3">
              <div className="flex flex-col items-center"><input type="checkbox" className="h-4 w-4 rounded text-sky-400" /><span className="text-[10px] font-black text-slate-400 mt-1">L</span></div>
              <div className="flex flex-col items-center"><input type="checkbox" className="h-4 w-4 rounded text-sky-400" /><span className="text-[10px] font-black text-slate-400 mt-1">R</span></div>
            </div>
          )}
        </div>
      </td>
