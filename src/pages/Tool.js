...
function Tool() {
  ...
  const charRows = Object.keys(charData).flatMap((groupName) => {
    ...
    visibleItems.forEach(([charName, data]) => {
      rows.push(<Row key={`${groupName}-${charName}`} rowData={[charName, data]} isChecked={!!checkedItems[charName]} onToggle={(val) => handleToggle(charName, val, data)} headerLength={headerKeys.length} showHighlights={showHighlights} />);
    });
      ...
      rows.push(
        <tr key="fiti-link" className="bg-sky-50 print:hidden">
          <td colSpan={headerKeys.length + 2} className="p-4 border border-slate-700 text-center align-middle bg-white">
            <Link to="/fiti" className="text-xs font-black text-sky-700 hover:underline flex items-center justify-center gap-2 uppercase tracking-wide">Perform Modular FITI Assessment</Link>
          </td>
        </tr>
      );
    }
    ...
    <div className="p-4 md:p-10 max-w-[1600px] mx-auto min-h-screen bg-white font-sans text-slate-900 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10 no-print">
        <div className="p-6 bg-sky-50 rounded-3xl border border-sky-100 flex items-start gap-4">
          ...
          <p className="text-xs font-bold text-sky-800 leading-relaxed uppercase tracking-wide">
            Hover over blue "i's" for tasks. Toggle the button to hide or reveal highlighted results during task administration.
          </p>
        </div>
        <button 
      ...
      <footer className="mt-24 pt-12 border-t border-slate-100 text-center pb-16 no-print">
        ...
        <p className="text-[11px] text-slate-400 max-w-3xl mx-auto italic font-bold">
          © 2024-2026, Regents of the University of Colorado. Developed in the Colorado Motor Speech lab. All rights reserved. <br />
