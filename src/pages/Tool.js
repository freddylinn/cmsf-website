...
function Tool() {
  ...
  // RESTORED COMPREHENSIVE SMART PHRASE
  const generateSmartPhrase = () => {
    ...
    text += headerKeys.map((k, i) => {
        return `${k}: Net ${counts.Total[i]} (C:${counts.Yellow[i]} D:${counts.Green[i]} U:${counts.Red[i]})`;
    }).join('\n');
